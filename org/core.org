#+title: OpenACS on AltLinux using S6.
...
:CUSTOM_ID: about
:END:
This is [[https://www.openacs.org][OpenACS]] on [[https://hub.docker.com/_/alt][ALT Linux base docker image]] (version sisyphus) using [[https://github.com/just-containers/s6-overlay][s6-overlay]].   To install OpenACS I used  code from  [[https://github.com/gustafn/install-ns][Gustaf Neumann]] script [[https://github.com/gustafn/install-ns/blob/master/install-oacs.sh][install-oacs.sh]]. The base image is [[https://hub.docker.com/r/oupfiz5/alt-s6-openacs][oupfiz5/alt-s6-openacs]].
...
:END:
 ...
 |---------------------+-----------------------+---------------------------|
 | NS_IMAGE_NAME       | alt-s6-naviserver     | Set NaviServer image name |
 |---------------------+-----------------------+---------------------------|
 | IMAGE_NAME          | alt-s6-openacs            | Set OpenaACS image name   |
 |---------------------+-----------------------+---------------------------|
...
| OACS_IMAGE_NAME     | alt-s6-openacs         | Set OpenACS image name                    |
...
:CUSTOM_ID: ci-cd
:END:
   For  build and push docker images we use  [[https://github.com/oupfiz5/openacs-s6/blob/master/.github/workflows/on-push.yaml][GitHub Actions workflow]].
* Maintenance
:PROPERTIES:
...
#+begin_src sh :shebang ""
  CONTAINER_NAME="${CONTAINER_NAME:-alt-s6-openacs}"
  NS_IMAGE_REPOSITORY="${NS_IMAGE_REPOSITORY:-oupfiz5}"
  NS_IMAGE_NAME="${NS_IMAGE_NAME:-alt-s6-naviserver}"
  NS_IMAGE_TAG="${NS_IMAGE_TAG:-4.99.24}"
  IMAGE_NAME="${IMAGE_NAME:-alt-s6-openacs}"
...
jobs:
  checks:
    ...
    steps:
      ...
      - name: s6v3. OpenACS. Build. Check image and container.
        run: |
          echo "::group::Build image"
          set -a; source ${GITHUB_WORKSPACE}/src/VERSIONS ; set +a;
          echo "Build image"
...
LABEL \
    ...
    org.opencontainers.image.url="https://chiselapp.com/user/oupfiz5/repository/alt-s6-openacs/home" \
    org.opencontainers.image.documentation="https://chiselapp.com/user/oupfiz5/repository/alt-s6-openacs/wiki" \
    org.opencontainers.image.source="https://chiselapp.com/user/oupfiz5/repository/alt-s6-openacs/brlist" \
...
CMD []
#+end_src
** rootfs/etc/s6-overlay/scripts/wait-postgres-onetime.sh :code:
:PROPERTIES:
:CUSTOM_ID: rootfs-etc-cont-init-d-01-wait-postgres-onetime
:END:
...
** rootfs/etc/s6-overlay/s6-rc.d/wait-postgres-onetime/type :code:
#+begin_src sh :shebang "" :tangle-mode (identity #o644)
oneshot
#+end_src
** rootfs/etc/s6-overlay/s6-rc.d/wait-postgres-onetime/up :code:
#+begin_src sh :shebang "" :tangle-mode (identity #o755)
#!/command/execlineb -P
...
** rootfs/etc/s6-overlay/s6-rc.d/user/contents.d/wait-postgres-onetime :code:
#+begin_src sh :shebang "" :tangle-mode (identity #o644)

#+end_src
** rootfs/etc/services.d/naviserver/run :notangle:code:
:PROPERTIES:
:CUSTOM_ID: rootfs-etc-services-d-naviserver-run
:END:
#+begin_src sh :shebang "" :tangle no :tangle-mode (identity #o644)
#!/command/execlineb -P
##!/usr/bin/with-contenv bash
...
/usr/local/ns/bin/nsd -f -t /usr/local/ns/conf/openacs-config.tcl -u nsadmin
#+end_src

** rootfs/etc/s6-overlay/s6-rc.d/naviserver/type :code:
#+begin_src sh :shebang "" :tangle-mode (identity #o644)
longrun
#+end_src

** rootfs/etc/s6-overlay/s6-rc.d/naviserver/run :code:
#+begin_src sh :shebang "" :tangle-mode (identity #o755)
#!/command/execlineb -P
...
** rootfs/etc/s6-overlay/s6-rc.d/naviserver/finish :code:
#+begin_src sh :shebang "" :tangle-mode (identity #o755)
#!/bin/sh
...
** rootfs/etc/s6-overlay/s6-rc.d/user/contents.d/naviserver :code:
#+begin_src sh :shebang "" :tangle-mode (identity #o644)

#+end_src
** rootfs/etc/s6-overlay/s6-rc.d/naviserver/dependencies.d/wait-postgres-onetime :code:notangle:
#+begin_src sh :shebang "" :tangle-mode (identity #o644)
...
OACS_IMAGE_NAME="${OACS_IMAGE_NAME:-alt-s6-openacs}"
